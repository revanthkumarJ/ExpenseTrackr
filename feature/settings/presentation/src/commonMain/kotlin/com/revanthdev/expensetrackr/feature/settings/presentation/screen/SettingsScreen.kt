...
import expensetrackr.core.presentation.generated.resources.error_no_app_found
...
fun SettingsScreen(state: SettingsState, onAction: (SettingsAction) -> Unit) {
    ...
    Scaffold(
        ...
        LazyColumn(modifier = Modifier.padding(padding).fillMaxSize(), contentPadding = PaddingValues(bottom = 16.dp)) {
            ...
            item { SettingsItem(Icons.Rounded.Download, stringResource(Res.string.settings_downloads), onClick = { onAction(SettingsAction.OnDownloadsClick) }) }
            item { SettingsSectionHeader(stringResource(Res.string.settings_section_app_info)) }
            item {
                SettingsItem(
                    Icons.Rounded.Share,
                    stringResource(Res.string.settings_share),
                    onClick = {
